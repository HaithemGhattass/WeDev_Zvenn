...
class SecurityController extends AbstractController
{
 ...
 $this->get('session')->clear();
    ...
    function Updatepsw(Request $request,UserPasswordEncoderInterface $pass)
    {
        if($request->isMethod('POST')){
            ...
            if($request->request->get('pass')==$request->request->get('pass2')){
                ...
                $em->flush();
                $this->addFlash('success','mot de passe a mise a jour');
                return $this->redirectToRoute('app_profile');
            }
    ...
    function Verifsms(Request $request)
    {
        ...
        if($request->isMethod('POST')){
            ...
            if($request->request->get('smscode')==$user->getPhoneCode()){
                ...
                $em->flush();
                $this->addFlash('success','votre numero de telehpone est verifié');
                return $this->redirectToRoute('app_profile');
            }
            else{
    ...
    /**
     * @Route ("user/profile/reservation", name="profileReservation")
     */
    function affichagerec()
    {

        return $this->render('Frontend/ReservationProfile.html.twig');
